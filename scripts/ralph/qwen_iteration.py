...
Exit codes (the supervisor routes on these):
  ...
  42 current phase complete, or all stories passed
...
def current_phase() -> str | None:
    """Active manager-approved phase, or None for legacy unphased projects."""
    try:
        value = PHASE_PATH.read_text(encoding="utf-8").strip()
    except OSError:
        return None
...
def next_story(prd: dict) -> dict | None:
    ...
    blockedBy ids all pass. When PHASE exists, only that phase is eligible."""
    ...
    eligible = [
        ...
        for s in stories
        if not s.get("passes")
        and (phase is None or str(s.get("phase")) == phase)
...
def phase_complete(prd: dict) -> bool:
    """The active phase is green while later work remains."""
    phase = current_phase()
    if phase is None or all_passed(prd):
        return False
    stories = [s for s in prd.get("userStories", []) if str(s.get("phase")) == phase]
    return bool(stories) and all(s.get("passes") for s in stories)
...
def _run(event: dict) -> int:
    ...
    if story is None:
        if phase_complete(prd):
            print(f"PHASE-COMPLETE phase={current_phase()}", flush=True)
            return 42
        print(
            "no eligible story in the active phase: blocked or dependency-starved",
            flush=True,
        )
        return 43
